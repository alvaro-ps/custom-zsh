...
dataproc_logs() {
  ...
  gcloud logging read \
    "resource.type=\"cloud_dataproc_batch\" AND resource.labels.batch_id=\"$batch_id\" AND resource.labels.location=\"$region\" AND log_name=\"projects/$project/logs/dataproc.googleapis.com%2Foutput\"" \
    --project="$project" \
